...
Erstelle Zeilen direkt im AV-Skript mit **Add row** oder plane Einstellungen in der linearen Einstellungsliste – so oder so landest du bei denselben Zeilen: Das AV-Skript und die lineare Einstellungsliste bleiben synchron, in derselben Reihenfolge. Szenen-Header werden nicht als Zeilen dargestellt; die Tabelle ist eine flache Abfolge von Einstellungen. Einstellungen, die nur auf einem Grid-Board existieren, erscheinen nicht im AV-Skript – es folgt der linearen Liste (siehe [Linear vs Drehtage](/de/guides/shot-list/linear-vs-production) für den Unterschied).
...
*"Ich habe eine Beschreibung im AV-Skript umformuliert und die Einstellungsliste hat sich geändert."* Auch erwartet – die Beschreibung ist geteilt: Sie in einem der beiden Tools zu bearbeiten, aktualisiert beide.
