...
The CineLog AV Script is the classic two-column commercial and advertising script: Audio on the left, Visual on the right, one row per beat. Like the Storyboard, it is not a separate dataset — every row **is** a shot in your project's Linear Shot List, kept in sync in both directions. Each project has exactly one AV Script, and the whole tool is built around the client-review loop: write, time, send, collect per-row approvals, and iterate until final sign-off.
