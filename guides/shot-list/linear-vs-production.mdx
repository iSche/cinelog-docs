...
- **Add a banner.** In Production, `Cmd / Ctrl + N` adds a banner instead of a shot (keyboard shortcuts work on desktop and tablet layouts, not phones). On a phone, the `+` in the top app bar does the same thing, so a Production day gives you a banner where a Linear list would have given you a shot. Or use the call sheet schedule's **+ Add banner** action, which adds to the same production day, between whole scenes.
...
Every shot list, storyboard, AV Script and schedule PDF prints when it was made in its header, like `Exported 8/8/26 02:45 PM`, in the same date and time formats the rest of the app is set to. On a set where three versions of the same document are going around, that line is how the crew tells which one is the current one.
