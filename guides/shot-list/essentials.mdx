...
- **Manually, one shot at a time.** In Linear view, click the `New Shot` button on the right of the top menu bar (desktop), or the `+` button in the top app bar (phone). Both open a dialog asking which scene number to add the shot to. Careful in Production view though: there the same button makes a **banner** rather than a shot, on the phone and on desktop alike, because a production day is a schedule and what you add to a schedule is an event. To put a new shot on a day, add it in Linear and then assign it the day.
