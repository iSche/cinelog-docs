...
- The client can't refresh the code themselves. When it expires, reopen the Share dialog — an expired code is replaced with a fresh one automatically, which you can copy for your client along with the link. Re-sending the email also delivers the fresh code, but on a **Comment & Approve** link that press of **Send for review** starts a new round and counts against the free plan's rounds (see below) — copying is the round-free fix.
...
On the free plan, each project gets **3 Send-for-review rounds**; paid plans are unlimited. **Every press of Send for review consumes a round** — including re-sending the link by email. Copying the link or changing link options never does. If you hit the cap, the app offers an upgrade.
...
On your side, editing rows and writing comments work fully offline — changes save locally and sync when you reconnect, like everything else in CineLog. Link management does not: creating, updating, revoking, and sending the link all require a connection.
