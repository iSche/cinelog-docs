...
The left side of the header carries your company profile (name, address, phone) and logo. Upload the logo once at the workspace level through the company profile dialog and it appears in every call sheet's header and on emailed PDFs.

A sheet with no company on it is a real sheet. If you haven't set one, that third of the header reads **Add company** rather than sitting there looking broken, and the exported PDF simply leaves the space empty. Clearing the company off a project does the same thing, so removing the wrong letterhead is a real option rather than something you have to work around.
...
If the call sheet has a location with coordinates, CineLog fetches the day's forecast automatically the first time you open the sheet. Tap the weather block to open the full card.

The card splits into a **DAY** column and a **NIGHT** column, and each column carries the same four readings:

- Temperature, with the feels-like reading under it
- The conditions, with an icon
- Precipitation chance, named by type when there is one, like "Rain 40%"
- Wind speed and direction, with gusts when they're forecast, like "12 mph NW · gusts 24"

Under the two columns sit three readings for the day as a whole: UV index, cloud cover, and humidity. Below those, **SUN** and **MOON**, each with a rise time and a set time, and the moon's phase written under its times.

All of those times follow your 12h / 24h display setting, so a sheet set to 24-hour prints sunrise as 06:14 rather than 6:14 AM. The temperature unit follows the display setting too (°F / °C).

There's no manual weather entry. What the card shows is what the forecast service returned, and the one control on it is **Refresh Weather**, which fetches again for the same location and date. Forecasts reach about ten days out, so a sheet dated further ahead than that shows a note instead of numbers and fills itself in once the date comes into range.
...
On your own Account screen, tapping your avatar gives you **Change photo**, **Reposition** and **Remove**. Reposition reopens the cropper on the original photo you uploaded rather than on the small version the app draws, so you can reframe the same picture as many times as you like without it getting softer each pass.
...
*"Weather didn't load."* The auto-fetch needs coordinates, so set the location through the address autocomplete rather than typing it in free-hand, because only the picked result carries a lat/lng. Then reopen the call sheet. If it's still blank, tap the weather block and press **Refresh Weather**. A date more than about ten days out has no forecast yet, and the card says so rather than leaving you guessing.
