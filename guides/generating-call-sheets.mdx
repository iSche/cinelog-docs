...
**Google Places autocomplete** is wired into every address field. Start typing and you'll see suggested places; picking one populates the address and stores the coordinates so the weather lookup (below) has a location to work with.
...
Inserting a banner mid-scene splits the surrounding scene at the banner — see [Linear vs Production](/guides/shot-list/linear-vs-production) for the full split-and-merge behavior.
...
If a row's explicit start time conflicts with what the cascade would have produced (you've over-scheduled), the time is highlighted **red**. If there's a gap between rows (the cascade has slack), it shows **yellow**. Either way, the row's own estimated duration is what controls the next row, not the gap.
...
Every person row in Talent, Crew, or Clients can carry private notes. These notes are visible to the producer in the call sheet UI, but on the emailed PDF they appear **only on that person's copy** — not on anyone else's. Useful for individual instructions ("park at side entrance," "wardrobe fitting before call").
...
- Their per-person notes, if any (appended to the email body, not on the PDF visible to others).
...
*"The schedule time is highlighted red / yellow."* Red means you've over-scheduled — the explicit start time on that row contradicts what the time cascade computed. Yellow means there's a gap between rows that the cascade left unfilled. Adjust the durations or the start time to reconcile.
